/**
 * FROZEN REFERENCE — do not edit. The line tracker as shipped in Gaze Reader 1.0
 * (src/reading/lineTracker.ts at commit 93a21df), before the lighting-offset work:
 * ±1.5 lines of modelled drift, page turns that keep the old drift belief, an
 * off-text filter on the single drift estimate, 5 % uniform remainder on
 * forward/regression transitions. Its saccade classifier is frozen below too, so
 * the benchmarks keep comparing against exactly what shipped.
 */
import type {
  Fixation,
  GazeSample,
...
export interface LegacyLineTrackerOptions {
...
export const LEGACY_LINE_TRACKER_OPTIONS: Readonly<LegacyLineTrackerOptions> = Object.freeze({
  sigmaYLines: 0.9,
  driftRate: 0.15,
  maxDriftLines: 1.5,
});
...
interface TrackedLineEstimate extends LineEstimate {
...
const LINE_TRANSITIONS: Readonly<Record<Exclude<SaccadeKind, 'jump'>, Readonly<TransitionRow>>> = Object.freeze({
...
export class LegacyLineTracker {
  private readonly opts: LegacyLineTrackerOptions;
  ...
  constructor(opts: Partial<LegacyLineTrackerOptions> = {}) {
    const d = LEGACY_LINE_TRACKER_OPTIONS;
...
function findLineByDocTop(lines: readonly TextLine[], docTop: number, tol: number): number {
  let lo = 0;
...
// ───────────────────────────── Saccade classification (frozen copy of src/signal/fixations.ts) ─────────────────────────────

const finite = (v: number): boolean => Number.isFinite(v);

/** Used when no layout is known: typical desktop reading metrics. */
const SACCADE_FALLBACK = Object.freeze({ linePitchPx: 40, columnWidthPx: 640 });
