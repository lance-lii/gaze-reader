/**
 * FROZEN REFERENCE — do not edit. The page-end detector as shipped in Gaze Reader 1.0
 * (src/reading/pageEndDetector.ts at commit 93a21df). The offset benchmarks run the
 * new tracker with this detector too, to isolate what the detector changes buy.
 */
import type {
  GazeSample,
  LineEstimate,
...
interface PageEndOptions {
  sensitivity: Sensitivity;
  glanceDownToTurn: boolean;
}

const DEFAULT_PAGE_END_OPTIONS: Readonly<PageEndOptions> = Object.freeze({
  sensitivity: 'balanced',
  glanceDownToTurn: true,
});

interface PageEndThresholds {
...
const PAGE_END_PRESETS: Readonly<Record<Sensitivity, Readonly<PageEndThresholds>>> = Object.freeze({
...
const PAGE_END_GUARDS = Object.freeze({
...
function lastFullyVisibleLine(layout: LineLayout): number {
  const lines = layout.lines;
  for (let i = lines.length - 1; i >= 0; i--) if (lines[i]!.fullyVisible) return i;
  return -1;
}

function pageEndZones(layout: LineLayout | null): PageEndZones | null {
...
export class LegacyPageEndDetector {
 ...
 */
function endProgress(line: TextLine, x: number, columnWidth: number): number {
